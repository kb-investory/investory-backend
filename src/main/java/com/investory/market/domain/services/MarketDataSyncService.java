...
public class MarketDataSyncService {
    ...
    private final KisMarketDataPort kisMarketDataPort;
    private final StockRepository stockRepository;
    private final StockPriceRepository stockPriceRepository;

    public MarketDataSyncService(KisMarketDataPort kisMarketDataPort,
                                  StockRepository stockRepository,
                                  StockPriceRepository stockPriceRepository) {
        this.kisMarketDataPort = kisMarketDataPort;
        this.stockRepository = stockRepository;
        this.stockPriceRepository = stockPriceRepository;
    }

    // KIS search-stock-info를 조회해서 stocks 테이블에 upsert한다.
    public Stock syncStockInfo(SyncStockCommand command) {
        StockInfoDto dto = kisMarketDataPort.fetchStockInfo(command.stockCode());
        Stock stock = Stock.create(dto);
        return stockRepository.save(stock);
    }

    // KIS inquire-price-2를 조회해서 stock_prices 테이블에 저장한다.
    // stocks에 해당 종목이 없으면(먼저 syncStockInfo를 안 했으면) 먼저 채우고 진행한다.
    public StockPrice syncDailyPrice(SyncStockCommand command) {
        Stock stock = stockRepository.findByStockCode(command.stockCode())
                .orElseGet(() -> syncStockInfo(command));

        StockPriceDto dto = kisMarketDataPort.fetchDailyPrice(command.stockCode());
        StockPrice stockPrice = StockPrice.create(stock.getSecurityId(), dto);
    ...
    public void syncStockAndPrice(SyncStockCommand command) {
        Optional<Stock> existing = stockRepository.findByStockCode(command.stockCode());
    ...
    // 종목 사이의 호출 속도 조절(초당 몇 건까지)은 KisMarketDataClient가 전담한다 - 여기서는 신경 쓸 필요 없다.
    public void syncAllTrackedStocks() {
        List<String> stockCodes = stockRepository.findAllStockCodes();
        log.info("일일 종목 데이터 배치 시작. 대상 종목 수={}", stockCodes.size());

        int successCount = 0;
        for (String stockCode : stockCodes) {
            if (syncWithRetry(stockCode)) {
                successCount++;
            }
        }

        log.info("일일 종목 데이터 배치 종료. 성공={}/{}", successCount, stockCodes.size());
    }
    ...
    private boolean syncWithRetry(String stockCode) {
        for (int attempt = 1; attempt <= MAX_ATTEMPTS_PER_STOCK; attempt++) {
            try {
                syncStockAndPrice(new SyncStockCommand(stockCode));
                return true;
            } catch (Exception e) {
                boolean isLastAttempt = (attempt == MAX_ATTEMPTS_PER_STOCK);
                if (isLastAttempt) {
                    log.error("종목 배치 갱신 최종 실패(재시도 {}회 소진). stockCode={}", MAX_ATTEMPTS_PER_STOCK, stockCode, e);
                } else {
                    log.warn("종목 배치 갱신 실패, 재시도합니다. stockCode={}, attempt={}/{}, cause={}",
                            stockCode, attempt, MAX_ATTEMPTS_PER_STOCK, e.getMessage());
