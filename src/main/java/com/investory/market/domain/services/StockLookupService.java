 ...
 * StockPriceRepository까지 딸려있음) cross-domain 조회 용도로는 안 맞는다. 이 서비스는:
 *  - 못 찾아도 예외를 던지지 않고 Optional/빈 리스트로 반환한다 (호출 측에서 스킵 등으로 처리하도록)
 *  - StockRepository에만 의존한다 (StockPriceRepository 등 불필요한 의존성 없음)
 *
...
public class StockLookupService {
    ...
    public StockLookupService(StockRepository stockRepository) {
    ...
    public List<Stock> findByIds(List<Long> securityIds) {
        return stockRepository.findBySecurityIds(securityIds);
    }
}
